...
- Need accession numbers for input, split paired data
...
- TrimGalore
    - Performs adapter removal and quality filtering via calling the Cutadapt tool
    - Cutadapt is here: ~/.local/bin/cutadapt 
    - ~/TrimGalore-0.6.6/trim_galore --path_to_cutadapt  ~/.local/bin/cutadapt 
- Trimmomatic
    - Trimmomatic has two trimming modes: ‘adapters and SW’ mode and ‘adapters and MI’ mode. 
    - In ‘adapters and SW’ mode, a sliding window approach is used to remove read bases that 
      have a low sequencing quality. In ‘adapters and MI’ mode, a maximum information quality 
      filtering approach is applied for removing low quality bases. Adapter sequences are 
      detected and removed in both modes.

# Alignment
- References
    - https://pubmed.ncbi.nlm.nih.gov/32138290/ 
...
- Abundance w/ tximport (for pseudoaligners)

# Normalization
- References
    - https://www.nature.com/articles/s41598-020-76881-x
    - Gene expression values were represented using the normalization techniques provided by 
      each algorithm: Fragments per Kilobase of Mapped reads (FPKM), Transcripts per Million 
      (TPM), Trimmed Mean of M values (TMM from edgeR), Relative Log Expression (RLE from 
      DESeq2), upper quartile (UQ), coverage (Cov), estimated counts (Est_Counts) and 
      effective counts (Eff_Counts). 
      
    
### Read this to set up short paths...
https://askubuntu.com/questions/402353/how-to-add-home-username-bin-to-path 

# Data
https://www.ncbi.nlm.nih.gov/bioproject/PRJNA751390
https://www.ncbi.nlm.nih.gov/sra?linkname=bioproject_sra_all&from_uid=751390
...
```{r DESeq2 significance distribution and lfcshrink, warning=FALSE}
...
mea_tumor <-  rowMeans(counts(de_final_df, normalized=TRUE)[, de_final_df$tissue == "Normal_fat"])
...
ggplot(data = lfc_comp, aes(x = pvalue)) + geom_histogram(binwidth = 0.01)
ggplot(data = lfc_comp, aes(x = padj)) + geom_histogram(binwidth = 0.01)


```

```{r DESeq2 significance and direction, warning=FALSE}
...
vol_plot <- ggplot(vol_df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  ggtitle("Tumor vs. Normal with DESeq2") + 
  theme(plot.title = element_text(hjust = 0.5)) +
